...
"""Sim-only teleoperation using pyroki IK — no robot hardware required.

Phone -> IK pipeline drives a virtual SO-101 displayed in viser.
No robot connection is attempted.

Usage:
    python teleoperate_pyroki_sim_only.py
"""
...
IK_VEL_WEIGHT = 0.1     # joint velocity smoothing (higher = smoother but more lag)
...
class PyrokiKinematicsAdapter:
    ...
    def _pyroki_rad_to_motor_deg(self, q_rad: np.ndarray, passthrough_deg: np.ndarray) -> np.ndarray:
...
def make_obs(sim_joints: dict[str, float]) -> RobotObservation:
    return {f"{name}.pos": sim_joints[name] for name in MOTOR_NAMES}
...
def main() -> None:
    ...
    sim_joints: dict[str, float] = dict(SIM_HOME_DEG)
    ...
    print("Starting sim-only pyroki teleop. Squeeze the controller trigger to teleoperate...")
